...
import deleteTodoItem from "./modules/utils/delete";
import LocalStorage from "./modules/local-storage";
import TodoItem from "./modules/todoitem";
import addToDOM from "./modules/utils/add";
import sortAndUpdate from "./modules/utils/sort-and-update";
import {
  createFragments,
  displayClock,
  displayDate,
} from "./modules/display-time";

fetch(`https://picsum.photos/${window.screenX}/${window.screenY}`)
  .then((img) => {
    console.log(img);
    document.querySelector("body").style = "background: url(" + img.url + ")";
  })
  .then(() => {
    run();
  });
const run = () => {
  ...
  const clockElements = clockContainer.children;
  createFragments(clockElements[0]);

  setInterval(() => {
    displayClock(clockElements[0]);
    displayDate(clockElements[1]);
  }, 1000);
